...
const lookup = (key, formatter) => ({
  ...
  render: truncate(hash => (formatter ? formatter(hash[key]) : hash[key]))
});
...
exports.fileList = label => ([
  ...
  lookup(label, list => (list ?
    `(${list.length}) ${list.map(String).map(el => path.basename(el)).join(', ')}` :
    '-')
  )
]);
...
exports.multiline = (...lines) => {
  const width0 = lines.reduce((max, [first]) => Math.max(max, first.length), 0);
