...
const promiseLense = ({parallelism, progress}) => {
  ...
  const factory = (labels = []) => {
    ...
    const merge = (...fns) => (args, context = self) => {
      const mergeObjects = (reduced1, obj = {}) =>
        Object.keys(obj)
          .filter((k) => (obj[k] !== args[k]) && (obj[k] !== reduced1[k]))
          .reduce((reduced2, k) => Object.assign(reduced2, {
            [k]: [reduced2[k], obj[k]].every(Array.isArray) ? reduced2[k].concat(obj[k]) : obj[k]
          }), reduced1);

      return context.all(fns, (fn) => fn(args, context))
        .then((results) => results.reduce(mergeObjects, {}))
        .then((merged) => Object.assign({}, args, merged));
    };
