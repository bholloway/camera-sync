...
const scanMedia = (exifFn, {whitelist = []} = {}) => (absolute) =>
  Promise.all([
    Promise.all([
      exifFn(absolute),
    ...
    .then(([{createdDate, dateInfo, makeModel, birthtime}, checksum]) => {
      const ext = extname(absolute).slice(1).toLowerCase();
      const date = dateInfo ? createdDate : whitelist.includes(ext) ? birthtime : null;
      return ({
        absolute,
        createdDate,
        dateInfo,
        birthtime,
        checksum,
        cannonical: date ? name({absolute, date, makeModel, checksum}) : null,
        toString: () => absolute
      });
    });
